...
public class DetailKalender extends AppCompatActivity {
    DataHelper dbHelper;
    protected Cursor todo, todoPagi, todoSiang, todoMalam;

    ExpandableListKalender listAdapter;
...
//        });
        ...
        listAdapter = new ExpandableListKalender(this, listDataHeader, listDataChild);
        ...
        header.setText(getIntent().getStringExtra("tanggal"));
...
//        String date = df.format(calendar.getTime());
        ...
        todo = db.rawQuery("SELECT * FROM todo WHERE tanggal='"+date+"'", null);
        todo.moveToFirst();
        if(todo.getCount()>0){
            int child = 0;
            // Adding child data
            ...
            if(todoPagi.getCount() > 0 ){
                ...
                for(int i=0; i<todoPagi.getCount(); i++){
                    ...
                    if(todoPagi.getString(4).substring(0,9).equals("Tambahkan")){
                        pagi.add(todoPagi.getString(4));
                    } else {
                        pagi.add(todoPagi.getString(4).substring(9));
                    }
                }
            ...
            if(todoSiang.getCount()>0){
                ...
                for(int i=0; i<todoSiang.getCount(); i++){
                    ...
                    siang.add(todoSiang.getString(4).substring(10));
                }
            ...
            if(todoMalam.getCount()>0){
                ...
                for(int i=0; i<todoMalam.getCount(); i++){
                    ...
                    malam.add(todoMalam.getString(4).substring(10));
                }
